...
void mergeSort(int s, int e, vector<pair<pair<int,int>,int>>&a, vector<int>&inversions, vector<int>&ans2){
    if(s>=e){
        return;
    }
    int mid = s+(e-s)/2;
    ...
    while(left<=mid && right<=e){
        if(a[left].first.second >= a[right].first.second){
            ans2[a[right].second] += mid-left+1;
            merged[ptr++] = a[right++];
            cnt++;
        }
        else{
            merged[ptr++] = a[left];
            inversions[a[left++].second] += cnt;
        }
    }
    while(left<=mid){
        inversions[a[left].second] += cnt;
